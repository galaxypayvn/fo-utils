	...
	if ok {
		...
		if data.Returns != nil && len(data.Returns) > 0 {
			for k, v := range data.Returns {
				...
				if qq, ok := ox.driver.ToSQLValueQuery(vx, nil); ok {
		...
		for _, v := range data.Datas {
			...
			for _, v2 := range cols {
				if cur, ok := v[v2]; ok {
					if res, ok := ox.driver.ToSQLValueQuery(cur, tb.Columns[v2]); ok {
						val = append(val, res)
						continue
					}
				}
		...
		// process values
		{
			for k, v := range data.Datas[0] {
				if col, ok := tb.Columns[k]; ok {
					if val, ok := ox.driver.ToSQLValueQuery(v, tb.Columns[k]); ok {
...
func (ox sqlQuery) BatchUpdateTable(table string, args TableData) (qo string, errx serror.SError) {
	if len(args.Datas) <= 0 {
		errx = serror.Newc(fmt.Sprintf("Data is empty"), "while sqlq_batch_update")
		return
	}

	bindingArgs := map[string]string{
		"!table":      fmt.Sprintf("__:~%s__", table),
		"!columns":    "",
		"!values":     "",
		"!sets":       "",
		"!conditions": "",
		...
		// conditions
		{
			...
			for k, v := range args.Conditions {
				...
				if cur, ok := tb.Columns[kx]; ok {
					stx, ok := driver.ToSQLConditionQuery(QColumn([]string{cur.Name}), opr, v)
					if !ok {
		...
		// values
		{
			...
			for k, v := range args.Datas {
				...
				for k2, v2 := range cols {
					...
					cur, ok := driver.ToSQLValueQuery(v[v2], nil)
					if !ok {
			...
			for k, v := range data.Conditions {
				...
				if cur, ok := tb.Columns[kx]; ok {
					...
					if !isDynamic && !isNotToUpdate {
						val, _ := ox.driver.ToSQLValueQuery(v, nil)
		...
		// process rows
		{
			for _, v := range data.Datas {
				...
				for k2, v2 := range keys {
					...
					if cur, ok := v[v2]; ok {
						if val, ok := ox.driver.ToSQLValueQuery(cur, nil); ok {
							vx = append(vx, val)
							continue
						}
		...
		case DriverPostgreSQL:
			...
			// process conditions
			{
				for k, v := range data.Conditions {
					...
					case "@":
						conq, ok := ox.driver.ToSQLValueQuery(v, nil)
						if !ok {
		...
		for _, v := range data.Data {
			...
			for _, v2 := range keys {
				if cur, ok := data.Meta[v2]; ok {
					if v3, ok := ox.driver.ToSQLValueQuery(cur, nil); ok {
						val = append(val, v3)
						continue
					}
				}
			...
			for _, v2 := range eavs {
				...
				if v3, ok := ox.driver.ToSQLValueQuery(vc, nil); ok {
					val = append(val, v3)
					continue
				}
				val = append(val, "")
		...
		case DriverPostgreSQL:
			...
			if tb.SoftDelete != "" {
				...
				if ccol, ok := tb.Columns[tb.SoftDelete]; ok {
					if cval, ok := ox.driver.ToSQLValueQuery(now, nil); ok {
			...
			for k, v := range dels {
				...
				if ccol, ok := tb.Columns[k]; ok {
					if cval, ok := ox.driver.ToSQLValueQuery(v, nil); ok {
			...
			for _, v := range data.Data {
				...
				for _, v2 := range keys {
					if utarray.IsExist(v2, fields) {
						if res, ok := ox.driver.ToSQLValueQuery(data.Meta[v2], nil); ok {
					...
					if res, ok := ox.driver.ToSQLValueQuery(cv, nil); ok {
						vals = append(vals, res)
						continue
					}
				}
