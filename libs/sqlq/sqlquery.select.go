...
import (
	...
	"bitbucket.org/finesys/finesys-utility/utils/utint"
	...
	for k, v := range conds {
		...
		if colc, ok := argConds[v.Key]; ok {
			...
			// not referenced
			{
				...
				colVal, ok := ox.driver.ToSQLValueQuery(colc.Value, nil)
				if !ok {
		...
		for _, v2 := range prfx[":#"] {
			...
			case "conds":
				...
				if v4, ok := args.Conditions[strings.Join(v2sp[1:], ".")]; ok {
					if v5, ok := ox.driver.ToSQLValueQuery(v4, nil); ok {
						v3 = v5
					}
				}
		...
		case "eav":
			...
			if vcs, ok := usedVirtualColumns[v.Key]; ok {
				...
				for _, v2 := range vcs {
					...
					colNmVal, _ := ox.driver.ToSQLValueQuery(colNm, nil)
					...
					if colGrp != "" {
						colGrpVal, _ := ox.driver.ToSQLValueQuery(colGrp, nil)
